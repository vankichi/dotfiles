#!/usr/bin/env bash
# Claude Code status line — mirrors key elements of Starship config
...
# --- git branch ---
branch=$(git -C "$(echo "$input" | jq -r '.cwd // "."')" --no-optional-locks rev-parse --abbrev-ref HEAD 2>/dev/null)
if [ -n "$branch" ]; then
