...
**If launched with a cwd inside a worktree** (e.g., a parallel-cycle collision): EnterWorktree cannot create a nested worktree from inside one. Identify the main checkout and create your own worktree off main (or origin/main) with `git worktree add`, then move into it. Never touch files inside another agent's worktree. If Edit/Write to the newly created worktree keeps being rejected (under subagent cwd pinning, `EnterWorktree(path)` may report success while the write boundary does not actually move — 2026-07-15 FB): clean up the new worktree with `git worktree remove`, then move to the branch swap. Before swapping, mechanically confirm the original worktree is not owned by another active cycle — the check is a reverse lookup: grep the state files in the per-project plans dir; if an active state file records this worktree path under `worktree:`, treat it as owned (recent mtime as a secondary signal) — if it is owned, do not swap; escalate and stop (the "never touch another agent's worktree" principle). Once confirmed safe, run `git fetch origin <base-ref>` to refresh the remote-tracking ref, then continue by swapping only the branch inside the pinned original worktree directory via `git checkout -b <branch> origin/<base-ref>` (the original branch's commits are preserved).
