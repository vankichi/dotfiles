...
description: The apply side of the knowledge loop — aggregates unprocessed insights across all projects, then reconciles against current state → decides where to apply (memory → rules → skill → agent) → implements the improvement → all the way to a draft improvement PR against dotfiles. Makes no design decisions (undecided items go back to a human as a decisions-needed list). The collection side is retrospect (its paired skill).
