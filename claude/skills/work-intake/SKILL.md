---
name: work-intake
description: Entry-point skill for the dev loop that picks up a ready ticket from Notion, validates it against the spec contract (rules/spec-contract.md), and returns a normalized work item. Tickets that fail the contract are skipped with a reason comment. The watched DB is resolved from a memory reference.
