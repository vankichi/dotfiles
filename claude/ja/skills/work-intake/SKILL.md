---
name: work-intake
description: Notion の ready ticket を拾い、spec contract (rules/spec-contract.md) を検証して正規化 work item を返す Dev loop の入口 skill。contract を満たさない ticket は skip + 理由コメント。watch 対象 DB は memory reference から取得。
...
- option: 引数に ticket URL が渡された場合は Step 2 の列挙を skip し、その ticket だけを対象にする。**URL 指定の ticket が既に「進行中」の場合は再開として扱い、contract 検証と work item 出力のみ行う** (status 遷移と skip コメントは省略)
