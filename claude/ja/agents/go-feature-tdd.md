...
Go module が初期化済み (`go.mod` が repo root にある) / DDD + Clean Architecture の典型レイアウト採用 / `go test ./...` が動く状態。
...
2. **実際のレイアウトを確認する** (`find internal -type d -maxdepth 3`) — project ごとに異なる: `domain/` 配下が `model/` か `entity/` か / `ports/` の場所 (`domain/ports/` か `application/ports/` か) / `application/` の usecase 分類 / `adapters/` の分類規則
...
**domain → ports/application → adapters** の順に、各層で以下を回す:
...
| ports / application | **port は手書き mock を介す** (実 adapter を呼ばない)。mock は同じ `_test.go` 内か `<port>_mock_test.go` に手書き |
...
3. **port は mock を介して test する**: usecase の test で実 adapter を呼ばない (flaky 化 / adapter 変更で usecase test が壊れる / 不要な latency)
