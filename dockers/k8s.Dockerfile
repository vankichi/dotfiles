...
RUN set -x; cd "$(mktemp -d)" \
	...
	&& curl -fsSLo ${BIN_PATH}/kubectl "${GOOGLE}/kubernetes-release/release/$(curl -s ${GOOGLE}/kubernetes-release/release/stable.txt)/bin/${TARGETOS}/${ARCH}/kubectl" \
...
RUN set -x; cd "$(mktemp -d)" \
	...
	&& case "${TARGETARCH}" in "amd64") ARCH="amd64" ;; "arm64") ARCH="arm64" ;; *) echo "Unsupported TARGETARCH: ${TARGETARCH}"; exit 1 ;; esac \
	&& curl -fL https://app.getambassador.io/download/tel2/linux/${ARCH}/latest/telepresence -o ${BIN_PATH}/${BIN_NAME} \
	&& chmod a+x ${BIN_PATH}/${BIN_NAME}
...
# 	&& REPO="${ORG}/${BIN_NAME}" \
# 	&& VERSION="$(curl --silent "${API_GITHUB}/${REPO}/${RELEASE_LATEST}" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' | sed 's/v//g')" \
# 	&& case "${TARGETARCH}" in "amd64") ARCH="x86_64" ;; "arm64") ARCH="arm64" ;; *) echo "Unsupported TARGETARCH: ${TARGETARCH}"; exit 1 ;; esac \
...
RUN set -x; cd "$(mktemp -d)" \
	...
	&& VERSION="$(curl --silent "${API_GITHUB}/${REPO}/${RELEASE_LATEST}" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' | sed 's/v//g')" \
	&& case "${TARGETARCH}" in "amd64") ARCH="x86_64" ;; "arm64") ARCH="arm64" ;; *) echo "Unsupported TARGETARCH: ${TARGETARCH}"; exit 1 ;; esac \
	&& curl -fsSLO ${GITHUB}/norwoodj/helm-docs/${RELEASE_DL}/v${VERSION}/${BIN_NAME}_${VERSION}_Linux_${ARCH}.tar.gz \
	&& tar -zxvf ${BIN_NAME}_${VERSION}_Linux_${ARCH}.tar.gz \
	&& mv ${BIN_NAME} ${BIN_PATH}/${BIN_NAME}
...
RUN set -x; cd "$(mktemp -d)" \
	...
	&& curl -fsSLO https://github.com/kptdev/kpt/releases/download/v${VERSION}/kpt_linux_${ARCH}-${VERSION}.tar.gz \
	&& tar -zxvf kpt_linux_${ARCH}-${VERSION}.tar.gz \
	&& mv kpt ${BIN_PATH}/kpt \
